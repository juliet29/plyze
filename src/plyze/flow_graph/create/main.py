...
def make_flow_graph(case_data: CaseData, cardinal_expansion_factor: float):
    case = EZ(case_data.idf)

    external_nodes = make_external_nodes(case, case_data.sql, cardinal_expansion_factor)
    zones = make_zones_for_graph(case)
    edges = make_edges_for_graph(case, case_data.sql)
